            ...
            decoder=dict(
                ...
                transformerlayers=dict(
                    ...
                    operation_order=('self_attn', 'norm', 'self_attn', 'norm','cross_attn', 'norm',
                                     'ffn', 'norm'))),
            changifier=dict(
                type='MapCRChangifier',
                num_layers=2,
                embed_dims=_dim_,
                transformerlayers=dict(
                    type='MapCRChangifierLayer',
                    num_vec=num_vec+1,
                    num_pts_per_vec=fixed_ptsnum_per_pred_line,
                    attn_cfgs=[
                        dict(
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='/data/maasai/user/resun/nuScenes/per_corr/nuscenes_map_infos_temporal_train.pkl',
    ...
    val=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='/data/maasai/user/resun/nuScenes/per_corr/nuscenes_map_infos_temporal_val.pkl',
        val=True,
        seed=0,
