...
class MapCRHead(DETRHead):
    ...
    def __init__(self,
                ...
                #  num_vec=20,
                 ...
                 loss_pv_seg=dict(type='SimpleLoss',
                              pos_weight=2.13,
                              loss_weight=1.0),
                 loss_change=dict(type='SimpleLoss',
                              pos_weight=0.0,
                              loss_weight=0.0),
        ...
        super(MapCRHead, self).__init__(
            *args, transformer=transformer, **kwargs)
        self.code_weights = nn.Parameter(torch.tensor(
            self.code_weights, requires_grad=False), requires_grad=False)
        self.loss_pts = build_loss(loss_pts)
        self.loss_dir = build_loss(loss_dir)
        self.loss_change = build_loss(loss_change)
        ...
        if not self.as_two_stage:
            ...
            if self.query_embed_type == 'all_pts':
                self.query_embedding = nn.Embedding(self.num_query,
                                                    self.embed_dims * 2)
            elif self.query_embed_type in ['instance_pts', 'gt_pts', 'partial2_gt_pts', 'partial2_noisy5_gt_pts',
                                           'partial05_gt_pts', 'noisy5_gt_pts', 'noisy10_gt_pts',
                                           'shift1_gt_pts', 'shift1_gt_pts', 'noisy1_gt_pts', 'noisy1_gt_hybrid_pts',
                                           'noisy5_gt_hybrid_pts', 'gt_hybrid_sep_pts', 'gt_hybrid_pts', "noisy5_gt_linear_pts", "gt_linear_pts"]:
            ...
            if self.query_embed_type in ["noisy5_gt_linear_pts", "gt_linear_pts"]:
                self.gt_proj = nn.Linear(5, self.embed_dims)
    ...
    def init_weights(self):
        ...
        if self.query_embed_type in ["noisy5_gt_linear_pts", "gt_linear_pts"]:
            with torch.no_grad():
                self.gt_proj.weight[:5] = torch.eye(5)
                self.gt_proj.weight[5:] = 0
        # with torch.no_grad():
        #     self.instance_embedding.weight[:,:5] = torch.zeros_like(self.instance_embedding.weight[:,:5])
        #     self.pts_embedding.weight[:,:5] = torch.zeros_like(self.pts_embedding.weight[:,:5])
    ...
    def forward(self, mlvl_feats, lidar_feat, img_metas, prev_bev=None,  only_bev=False, gt_labels_3d=None, gt_bboxes_3d=None):
        """Forward function.
        Args:
        ...
        if self.query_embed_type == 'all_pts':
        ...
        elif self.query_embed_type == 'gt_linear_pts':
            # Format gt pts list
            gt_pts_list = [
                self.gt_proj(torch.cat([
                    gt_bboxes.fixed_num_sampled_points.to(device),
                    F.one_hot(gt_label, num_classes=3).to(device).unsqueeze(1).expand(-1,20,-1)
                ], dim=-1)) if len(gt_bboxes.instance_list)>0 else
                torch.zeros((1,20,self.embed_dims)).to(device).float() for gt_label, gt_bboxes in zip(gt_labels_3d, gt_bboxes_3d)
            ]

            # Pad out missing GTs (hidden dimension then num_vec) FLIPPED ORDER
            num_pts_gt = gt_pts_list[0].shape[1]

            pts_embeds_dyn = self.pts_embedding.weight[:,:self.embed_dims].unsqueeze(0)
            instance_embeds_dyn = self.instance_embedding.weight[:num_vec, :self.embed_dims].unsqueeze(1)
            object_query_dyn = (pts_embeds_dyn + instance_embeds_dyn).to(dtype)

            assert num_pts_gt == self.num_pts_per_vec
            object_query_embeds = torch.stack(
                [torch.cat([
                    gt_pts,
                    object_query_dyn[:-len(gt_pts)]
                ], dim=0) for gt_pts in gt_pts_list], dim=0
            )

            # Build positional encoding as usual
            pts_embeds_pos = self.pts_embedding.weight[:,self.embed_dims:].unsqueeze(0)
            instance_embeds_pos = self.instance_embedding.weight[:num_vec, self.embed_dims:].unsqueeze(1)
            object_query_pos = (pts_embeds_pos + instance_embeds_pos).to(dtype)
            object_query_pos = object_query_pos.unsqueeze(0).expand(bs, -1, -1, -1)

            object_query_embeds = torch.cat([
                object_query_embeds,
                object_query_pos
            ], dim=-1).flatten(1,2)
        elif self.query_embed_type == 'gt_hybrid_sep_pts':
            # Format gt pts list
            gt_pts_list = [
                torch.cat([
                    gt_bboxes.fixed_num_sampled_points.to(device),
                ...
                torch.zeros((1,20,5)).to(device).float() for gt_label, gt_bboxes in zip(gt_labels_3d, gt_bboxes_3d)
            ]
            ...
            gt_pts_list = [
                torch.cat([
                    gt_pts,
                    torch.zeros((len(gt_pts), num_pts_gt, self.embed_dims-5)).to(device)
                ], dim=-1) for gt_pts in gt_pts_list
            ]

            pts_embeds_dyn = self.pts_embedding.weight[:,:self.embed_dims].unsqueeze(0)
            instance_embeds_dyn = self.instance_embedding.weight[:num_vec, :self.embed_dims].unsqueeze(1)
            object_query_dyn = (pts_embeds_dyn + instance_embeds_dyn).to(dtype)
            object_query_o2o = object_query_dyn[:50]
            object_query_o2m = object_query_dyn[50:]

            assert num_pts_gt == self.num_pts_per_vec
            object_query_embeds = torch.stack(
                [torch.cat([
                    gt_pts,
                    object_query_o2o[:-len(gt_pts)],
                    object_query_o2m,
                ], dim=0) for gt_pts in gt_pts_list], dim=0
            )
        ...
        elif self.query_embed_type == 'gt_hybrid_pts':
            # Format gt pts list
            gt_pts_list = [
                torch.cat([
                    gt_bboxes.fixed_num_sampled_points.to(device),
        ...
        if only_bev:  # only use encoder to obtain BEV features, TODO: refine the workaround
            ...
            outputs = self.transformer(
                ...
                no_expand_query=True

        )

        bev_embed,depth, hs, init_reference, inter_references, change_preds = outputs
    ...
    def _get_target_single(self,
        ...
        Args:
            ...
            gt_correspondence (Tensor, optional): Known GT correspondences to bbox preds
                with shape (num_query,). Default None.
        Returns:
        ...
        if (gt_correspondence is not None) and (self.attribution):
            # Build GT assignment for known association
            known_gt_hits = (gt_correspondence != -1)
            known_gt_inds = gt_correspondence[known_gt_hits]
            known_bbox_inds = torch.tensor(range(num_bboxes)).to(device)[known_gt_hits]

            true_gt_assigned = bbox_pred.new_full((num_bboxes,), -1, dtype=torch.long)
            true_gt_assigned[known_bbox_inds] = known_gt_inds + 1
            true_assigned_labels = bbox_pred.new_full((num_bboxes,), -1, dtype=torch.long)
            true_assigned_labels[known_bbox_inds] = gt_labels[known_gt_inds]
            true_order_index = torch.zeros((num_bboxes, num_gts), dtype=torch.long).to(device)

            # Build tensors with unknown GT correspondences
            unknown_gt_mask = torch.ones((num_gts,),dtype=bool).to(device)
            unknown_gt_mask[known_gt_inds] = False
            unknown_gt_inds = torch.tensor(range(num_gts)).to(device)[unknown_gt_mask]
            unknown_bbox_mask = torch.ones((num_bboxes,),dtype=bool).to(device)
            unknown_bbox_mask[known_gt_hits] = False
            unknown_bbox_inds = torch.tensor(range(num_bboxes)).to(device)[unknown_bbox_mask]

            unknown_bbox_pred = bbox_pred[unknown_bbox_inds]
            unknown_cls_score = cls_score[unknown_bbox_inds]
            unknown_pts_pred = pts_pred[unknown_bbox_inds]
            unknown_gt_bboxes = gt_bboxes[unknown_gt_inds]
            unknown_gt_labels = gt_labels[unknown_gt_inds]
            unknown_gt_shifts_pts = gt_shifts_pts[unknown_gt_inds]

            # assign_result, order_index = self.assigner.assign(bbox_pred, cls_score, pts_pred,
            #                                      gt_bboxes, gt_labels, gt_shifts_pts,
            #                                      gt_bboxes_ignore)
            unknown_assign_result, unknown_order_index = self.assigner.assign(unknown_bbox_pred, unknown_cls_score, unknown_pts_pred,
                                                                              unknown_gt_bboxes, unknown_gt_labels, unknown_gt_shifts_pts,
                                                                              gt_bboxes_ignore)

            # Merge unknown assignment into known true assignments
            unknown_assign_hits = unknown_assign_result.gt_inds > 0
            unknown_adjusted = torch.zeros_like(unknown_assign_result.gt_inds).to(device)
            unknown_adjusted[unknown_assign_hits] = unknown_gt_inds[unknown_assign_result.gt_inds[unknown_assign_hits]-1]+1
            true_gt_assigned[unknown_bbox_inds] = unknown_adjusted
            true_assigned_labels[unknown_bbox_inds] = unknown_assign_result.labels

            if unknown_order_index is not None:
                true_order_index[unknown_bbox_inds.unsqueeze(1).expand(-1,unknown_gt_inds.size(0)), unknown_gt_inds.unsqueeze(0).expand(unknown_bbox_inds.size(0),-1)] = unknown_order_index
            order_index = true_order_index

            assign_result = unknown_assign_result
            assign_result.gt_inds = true_gt_assigned
            assign_result.labels = true_assigned_labels
            assign_result.num_gts = num_gts
        ...
        (labels_list, label_weights_list, bbox_targets_list,
         ...
         pos_inds_list, neg_inds_list) = multi_apply(
            self._get_target_single, cls_scores_list, bbox_preds_list,pts_preds_list,
             gt_labels_list, gt_bboxes_list, gt_shifts_pts_list, gt_bboxes_ignore_list, gt_correspondence_list)
    ...
    def loss_single(self,
        ...
        if gt_correspondence_list is None:
            gt_correspondence_list = [None for _ in range(num_imgs)]
        ...
        cls_reg_targets = self.get_targets(cls_scores_list, bbox_preds_list,pts_preds_list,
                                           ...
                                           gt_bboxes_ignore_list, gt_correspondence_list)
    ...
    def loss(self,
        ...
        if "change_preds" in preds_dicts:
            change_preds = preds_dicts['change_preds']
        else:
            change_preds = None
        ...
        all_gt_correspondence_list = [gt_correspondence for _ in range(num_dec_layers)]
        # import pdb;pdb.set_trace()
        ...
        losses_cls, losses_bbox, losses_iou, losses_pts, losses_dir = multi_apply(
            ...
            all_gt_bboxes_ignore_list, all_gt_correspondence_list)
        ...
        if (perturbed is not None) and (change_preds is not None):
            loss_change = self.loss_change(change_preds, torch.tensor(perturbed, dtype=torch.float).unsqueeze(-1))
            loss_dict['loss_change'] = loss_change
