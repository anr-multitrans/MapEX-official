...
class MapCR(MVXTwoStageDetector):
    """MapTR.
    Args:
    ...
    def forward_pts_train(self,
                          ...
                          gt_seg_mask=None,
                          gt_pv_seg_mask=None,
                          perturb_gt_bboxes_3d=None,
                          perturb_gt_labels_3d=None,
        ...
        outs = self.pts_bbox_head(
            pts_feats, lidar_feat, img_metas, prev_bev, gt_labels_3d=perturb_gt_labels_3d, gt_bboxes_3d=perturb_gt_bboxes_3d)
        ...
        losses_pts = self.pts_bbox_head.loss(*loss_inputs, img_metas=img_metas, gt_correspondence=gt_correspondence, perturbed=perturbed)
    ...
    def forward_train(self,
        ...
        losses_pts = self.forward_pts_train(img_feats, lidar_feat, gt_bboxes_3d,
                                            ...
                                            gt_bboxes_ignore, prev_bev, gt_depth,gt_seg_mask,gt_pv_seg_mask, perturb_gt_bboxes_3d,
                                            perturb_gt_labels_3d, gt_correspondence, perturbed)
    ...
    def pred2result(self, bboxes, scores, labels, pts, attrs=None):
        ...
        return result_dict
    def simple_test_pts(self, x, lidar_feat, img_metas, prev_bev=None, rescale=False, gt_bboxes_3d=None, gt_labels_3d=None):
        """Test function"""
        outs = self.pts_bbox_head(x, lidar_feat, img_metas, prev_bev=prev_bev, gt_bboxes_3d=gt_bboxes_3d, gt_labels_3d=gt_labels_3d)

        # Substitue detected true maps
        #change_preds = (torch.sigmoid(outs['change_preds']) > 0.5).float()

        #num_gts = len(gt_labels_3d[0])
        #if num_gts == 0 or change_preds==1:
        #    pass
        #else:
        #    num_bboxes = outs["all_cls_scores"].shape[2]
        #    cls_scores = torch.nn.functional.one_hot(gt_labels_3d[0],num_classes=3)*200-100
        #    cls_scores = torch.cat([cls_scores, torch.ones((num_bboxes-num_gts,3)).cuda()*(-100)], dim=0)
        #    gt_cls_scores = cls_scores.unsqueeze(0).unsqueeze(0).expand(6,-1,-1,-1)
        #    outs["all_cls_scores"] = outs["all_cls_scores"]

        #    bboxes_preds = torch.cat([gt_bboxes_3d[0].bbox, torch.zeros((num_bboxes-num_gts,4))], dim=0).cuda()
        #    bboxes_preds = normalize_2d_bbox(bboxes_preds, self.pts_bbox_head.pc_range)
        #    outs["all_bbox_preds"] = bboxes_preds.unsqueeze(0).unsqueeze(0).expand(6,-1,-1,-1)
        #    pts_preds = torch.cat([gt_bboxes_3d[0].fixed_num_sampled_points, torch.zeros((num_bboxes-num_gts, 20, 2))], dim=0).cuda()
        #    pts_preds = normalize_2d_pts(pts_preds, self.pts_bbox_head.pc_range)
        #    outs["all_pts_preds"] =  pts_preds.unsqueeze(0).unsqueeze(0).expand(6,-1,-1,-1,-1)
        ...
        # import pdb;pdb.set_trace()
        return outs['bev_embed'], bbox_results, outs['change_preds']

    def simple_test(self, img_metas, img=None, points=None, prev_bev=None, rescale=False,
                    gt_bboxes_3d=None, gt_labels_3d=None, perturb_gt_bboxes_3d=None, perturb_gt_labels_3d=None, perturbed=None, **kwargs):
        ...
        new_prev_bev, bbox_pts, change_preds = self.simple_test_pts(
            img_feats, lidar_feat, img_metas, prev_bev, rescale=rescale, gt_bboxes_3d=perturb_gt_bboxes_3d, gt_labels_3d=perturb_gt_labels_3d)

        for result_dict, pts_bbox in zip(bbox_list, bbox_pts):
            result_dict['pts_bbox'] = pts_bbox
            result_dict['pts_bbox']["perturbed"] = perturbed
            if change_preds is not None:
                result_dict['pts_bbox']["change_preds"] = change_preds
