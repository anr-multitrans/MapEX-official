...
@TRANSFORMER.register_module()
class MapCRPerceptionTransformer(BaseModule):
    """Implements the Detr3D transformer.
    Args:
        as_two_stage (bool): Generate query from encoder features.
            Default: False.
    ...
    def __init__(self,
                 ...
                 feat_down_sample_indice=-1,
                 **kwargs):
        super(MapCRPerceptionTransformer, self).__init__(**kwargs)
        if modality == 'fusion':
            self.fuser = build_fuser(fuser) #TODO
        # self.use_attn_bev = encoder['type'] == 'BEVFormerEncoder'
        self.use_attn_bev = 'BEVFormerEncoder' in encoder['type']
        self.encoder = build_transformer_layer_sequence(encoder)
        self.decoder = build_transformer_layer_sequence(decoder)
        if changifier is not None:
            self.changifier = build_transformer_layer_sequence(changifier)
        else:
            self.changifier = None
        self.embed_dims = embed_dims
        self.num_feature_levels = num_feature_levels
        self.num_cams = num_cams
        self.fp16_enabled = False
    ...
    def forward(self,
        ...
        change_preds=None
        if self.changifier is not None:
            change_preds = self.changifier(inter_states, num_vec=kwargs["num_vec"], num_pts_per_vec=kwargs["num_pts_per_vec"])

        return bev_embed, depth, inter_states, init_reference_out, inter_references_out, change_preds
