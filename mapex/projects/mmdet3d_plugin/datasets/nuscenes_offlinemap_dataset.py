...
class LiDARInstanceLines(object):
    ...
    def fixed_num_sampled_points(self):
        ...
        instance_points_tensor = instance_points_tensor.to(
                            dtype=torch.float32)
        if self.perturb in ["shift1", "noisy5"]:
            if self.noise is not None:
                instance_points_tensor = instance_points_tensor + self.noise
            else:
                instance_points_tensor = instance_points_tensor + torch.randn_like(instance_points_tensor )*5
    ...
    def shift_fixed_num_sampled_points_v2(self):
        ...
        for idx, instance in enumerate(self.instance_list):
            ...
            if shifts_num > final_shift_num:
                # index = np.random.choice(multi_shifts_pts.shape[0], final_shift_num, replace=False)
                index = np.random.choice(multi_shifts_pts.shape[0]-1, final_shift_num-1, replace=False)
                index = np.insert(index+1, 0, 0)
                multi_shifts_pts = multi_shifts_pts[index]
...
class VectorizedLocalMap(object):
    ...
    def gen_vectorized_samples_short(self, map_annotation, example=None, feat_down_sample=32):
        '''
...
class CustomNuScenesOfflineLocalMapDataset(CustomNuScenesDataset):
    ...
    def vectormap_pipeline(self, example, input_dict):
        ...
        if 'rng_0' in input_dict:
            if self.val:
                choice = self.seed
            else:
                choice = np.random.choice([i for i in range(10)])
            gt_p = input_dict["rng_{}".format(choice)]["pick_p"]
        else:
            gt_p = np.random.uniform()

        perturb_annotation = input_dict['annotation'] if 'annotation' in input_dict.keys() else input_dict['ann_info']
        perturb_annotation = copy.deepcopy(perturb_annotation)
        perturbed=False
        if (self.perturb is None) or gt_p < self.perturb_p:
            num_delete = len(perturb_annotation["divider"]) + len(perturb_annotation["ped_crossing"])
            num_original = len(perturb_annotation["divider"]) + len(perturb_annotation["ped_crossing"]) + len(perturb_annotation["boundary"])
            gt_correspondence = torch.tensor([i if (i< num_original) else -1 for i in range(50)])
            example["gt_correspondence"] = gt_correspondence
            perturb = None
        elif self.perturb == "mapmodex":
            annotation = input_dict['ann_info']
            perturb_annotation = input_dict['ann_info_{}'.format(choice)]
            perturb_annotation = copy.deepcopy(perturb_annotation)
            num_original = len(perturb_annotation["divider"]) + len(perturb_annotation["ped_crossing"]) + len(perturb_annotation["boundary"])
            gt_correspondence = copy.deepcopy(input_dict["correspondence_{}".format(choice)]["divider"])
            gt_correspondence += [corr+len(annotation["divider"]) if corr!=-1 else -1 for corr in input_dict["correspondence_{}".format(choice)]["ped_crossing"]]
            gt_correspondence += [corr+len(annotation["divider"])+len(annotation["ped_crossing"]) if corr!=-1 else -1 for corr in input_dict["correspondence_{}".format(choice)]["boundary"]]
            gt_correspondence += [-1] * (50-num_original)
            example["gt_correspondence"] = torch.tensor(gt_correspondence)
            perturb = "mapmodex"
            perturbed=True
        elif self.perturb == "onlybound":
            num_delete = len(perturb_annotation["divider"]) + len(perturb_annotation["ped_crossing"])
            num_original = len(perturb_annotation["divider"]) + len(perturb_annotation["ped_crossing"]) + len(perturb_annotation["boundary"])
            gt_correspondence = torch.tensor([i+num_delete  if (i+num_delete< num_original) else -1 for i in range(50)])
            example["gt_correspondence"] = gt_correspondence
            perturb_annotation["divider"] = []
            perturb_annotation["ped_crossing"] = []
            perturb = "onlybound"
            perturbed=True
        elif self.perturb == "noisy5":
            num_delete = len(perturb_annotation["divider"]) + len(perturb_annotation["ped_crossing"])
            num_original = len(perturb_annotation["divider"]) + len(perturb_annotation["ped_crossing"]) + len(perturb_annotation["boundary"])
            dists = np.linalg.norm(np.mean(input_dict["rng_{}".format(choice)]["noise"]*5,axis=-2),axis=-1)
            valid_gt = dists < 1.0
            gt_correspondence = torch.tensor([i if (i< num_original) and valid_gt[i] else -1 for i in range(50)])
            example["gt_correspondence"] = gt_correspondence
            perturb = "noisy5"
            perturbed=True
        elif self.perturb == "shift1":
            num_delete = len(perturb_annotation["divider"]) + len(perturb_annotation["ped_crossing"])
            num_original = len(perturb_annotation["divider"]) + len(perturb_annotation["ped_crossing"]) + len(perturb_annotation["boundary"])
            dists = np.linalg.norm(np.mean(input_dict["rng_{}".format(choice)]["noise"][:,:1,:],axis=-2),axis=-1)
            valid_gt = dists < 1.0
            gt_correspondence = torch.tensor([i if (i< num_original) and valid_gt[i] else -1 for i in range(50)])
            # gt_correspondence = torch.tensor([i if (i< num_original) else -1 for i in range(50)])
            example["gt_correspondence"] = gt_correspondence
        ...
        perturb_anns_results = self.vector_map.gen_vectorized_samples_short(perturb_annotation,
                     example=example, feat_down_sample=self.aux_seg['feat_down_sample'])


        perturb_anns_results['gt_vecs_pts_loc'].perturb = perturb
        if 'rng_0' in input_dict and self.perturb == "noisy5":
            perturb_anns_results['gt_vecs_pts_loc'].noise = torch.tensor(input_dict["rng_{}".format(choice)]["noise"]).float()*5
        if 'rng_0' in input_dict and self.perturb == "shift1":
            perturb_anns_results['gt_vecs_pts_loc'].noise = torch.tensor(input_dict["rng_{}".format(choice)]["noise"])[:,:1,:].float()*1
        ...
        # Perturbed map
        gt_vecs_label = to_tensor(perturb_anns_results['gt_vecs_label'])
        if isinstance(perturb_anns_results['gt_vecs_pts_loc'], LiDARInstanceLines):
            gt_vecs_pts_loc = perturb_anns_results['gt_vecs_pts_loc']
        else:
            gt_vecs_pts_loc = to_tensor(perturb_anns_results['gt_vecs_pts_loc'])
            try:
                gt_vecs_pts_loc = gt_vecs_pts_loc.flatten(1).to(dtype=torch.float32)
            except:
                # empty tensor, will be passed in train, 
                # but we preserve it for test
                gt_vecs_pts_loc = gt_vecs_pts_loc
        example['perturb_gt_labels_3d'] = DC(gt_vecs_label, cpu_only=False)
        example['perturb_gt_bboxes_3d'] = DC(gt_vecs_pts_loc, cpu_only=True)
    ...
    def get_data_info(self, index):
        ...
        if self.num_perturb > 0:
            for i in range(self.num_perturb):
                input_dict['ann_info_{}'.format(i+1)] = info['annotation_{}'.format(i+1)]

        input_dict['ann_info'] = info['annotation']
        for i in range(10):
            if 'rng_{}'.format(i) in info:
                input_dict['rng_{}'.format(i)] = info['rng_{}'.format(i)]
            if 'annotation_{}_correspondence'.format(i) in info:
                input_dict['correspondence_{}'.format(i)] = info['annotation_{}_correspondence'.format(i)]
            if 'annotation_{}'.format(i) in info:
                input_dict['ann_info_{}'.format(i)] = info['annotation_{}'.format(i)]
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            pred_anno['perturbed'] = torch.tensor(det['perturbed']).cpu().numpy()
            if 'change_preds' in det:
                pred_anno['change_preds'] = torch.tensor(det['change_preds']).cpu().numpy()
    ...
    def _evaluate_single(self,
        ...
        for metric in metrics:
            ...
            if 'change_preds' in gen_results[0]:
                detail['NuscChange/Accuracy'] = torch.mean(torch.tensor([gen_results[i]['perturbed'][0] == (torch.sigmoid(torch.tensor(gen_results[i]['change_preds']))[0,0]>0.5) for i in range(len(gen_results))]).float())*100
                detail['NuscChange/Accuracy'] = detail['NuscChange/Accuracy'].cpu().numpy()
